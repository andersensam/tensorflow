...
# Extract LLVM into a blank image to avoid this step on other builds
WORKDIR /tmp/staging
...
# Set CUDA_HOME to be referenced by LLVM later
ENV CUDA_HOME=/usr/local/cuda-12.8

# Grab the target version of LLVM and build it using the clang (LLVM 10)
RUN git clone --depth 1 --branch llvmorg-20.1.7 https://github.com/llvm/llvm-project.git && \
    mkdir -p llvm-project/build && \
    cd llvm-project/build && \
    CC=clang CXX=clang++ cmake ../llvm -G Ninja \
